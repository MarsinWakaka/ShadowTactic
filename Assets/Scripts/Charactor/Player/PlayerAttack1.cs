...
public class PlayerAttack1 : IState
{
    ...
    public PlayerAttack1(Player player)
    {
        this.player = player;
        m_FSM = player.m_FSM;
    ...
    public void OnEnter()
    {
        ...
        player.anim.Play(AnimTags.ATTACK1);
    ...
    public void OnUpdate()
    {
        ...
        else if (!attackCheck && player.isCheckHit(attackID))
        {
            player.StopCheckHit(attackID);
        }
        else if (attackCheck && !player.isCheckHit(attackID)) // 攻击阶段可以缓慢移动
        {
            //攻击检测
