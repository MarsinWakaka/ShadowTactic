...
public class MonkNinjaAttack1 : IState
{
    ...
    public MonkNinjaAttack1(BasePlayer basePlayer)
    {
    ...
    public void OnEnter()
    {
        ...
        _basePlayer.anim.Play(AnimTags.ATTACK1);
    ...
    public void OnUpdate()
    {
        ...
        else if (isReadyForNextPhase)
        {
            ...
            {
                m_FSM.TransitionStatus(actionType.attack2);//跳转动作2
            }
        ...
        else if (!attackCheck && _basePlayer.IsAttacking(attackID))
        {
            _basePlayer.StopAttack(attackID);
        }
        else if (attackCheck && !_basePlayer.IsAttacking(attackID)) // 攻击阶段可以缓慢移动
        {
            //攻击检测
